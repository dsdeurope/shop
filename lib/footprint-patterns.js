...
export const NICHE_FOOTPRINTS = {
  ...
  ecommerce: {
    classes: ['add-to-cart', 'product-price', 'product-title', 'cart-widget', 'woocommerce'],
    metas: ['og:type=product', 'product:price', 'product:availability'],
    scripts: ['woocommerce', 'shopify', 'prestashop', 'stripe', 'paypal'],
    urlPatterns: ['/product/', '/shop/', '/boutique/', '/category/', '/collections/'],
    schemaTypes: ['Product', 'Offer', 'AggregateRating', 'Review'],
  },
  local_seo: {
...
export function matchFootprint(extractedFp, nicheName) {
  ...
  // Scripts tiers (poids 25)
  maxScore += 25;
  const scriptMatches = niche.scripts.filter(s =>
    extractedFp.scripts?.some(es => es.includes(s))
  ).length;
